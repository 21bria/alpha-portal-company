...
const { data: page, pending, error } = await useAsyncData(
  () => `public-page-vision-mission-${route.fullPath}`,
    ...
    <div class="relative mx-auto grid max-w-7xl gap-16 lg:grid-cols-[240px_minmax(0,1fr)]">
      ...
      <section class="relative overflow-hidden px-10">
        ...
        <div v-else>
          ...
          <div
            v-if="contentSection?.image_url"
            class="mt-8 hidden overflow-hidden rounded-[1rem] bg-zinc-100 md:mt-14 md:block"
          >
            <img
              :src="contentSection.image_url"
              :alt="contentSection.image_alt || contentSection.title"
              class="h-[320px] w-full object-cover object-center xl:h-[380px]"
            />
          </div>

          <div
            v-if="contentSection?.content"
            v-html="contentSection.content"
          />
          ...
          <div v-if="valuesSection && values.length" class="mt-12">
            ...
            <div
              v-if="valuesSection.image_url"
              class="relative mt-8 flex justify-start"
            >
              <img
                :src="valuesSection.image_url"
                :alt="valuesSection.image_alt || valuesSection.title"
            ...
            <div class="mt-10 max-w-3xl space-y-3">
              <div
                v-for="point in currentValue?.points ?? []"
                :key="point"
                class="flex gap-4"
              >
