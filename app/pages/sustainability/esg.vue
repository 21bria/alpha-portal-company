...
const { data: newsData } = await useAsyncData(
  () => `esg-latest-news-${route.fullPath}`,
  () =>
...
<template>
  <main class="relative min-h-screen overflow-hidden bg-white px-6 pt-36 pb-24 text-black">
    <div class="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_85%_18%,rgba(16,185,129,0.10),transparent_28%)]" />
    <div class="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_10%_80%,rgba(6,182,212,0.08),transparent_30%)]" />
    <div class="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_50%_40%,rgba(245,158,11,0.06),transparent_35%)]" />
    ...
    <div class="relative mx-auto grid max-w-7xl gap-16 lg:grid-cols-[240px_minmax(0,1fr)]">
      ...
      <section v-else class="px-0 lg:px-10">
        <h1 class="mt-4 bg-gradient-to-r from-emerald-400 via-lime-300 to-cyan-400 bg-clip-text text-3xl font-black tracking-tight text-transparent md:text-4xl">
          {{ section.title }}
        </h1>
        ...
        <div v-if="approaches.length" class="mt-20">
          ...
          <div class="mt-8 space-y-5">
            <div
              v-for="point in approaches"
              :key="point.title"
              class="border-l-2 border-emerald-400 pl-5"
            >
