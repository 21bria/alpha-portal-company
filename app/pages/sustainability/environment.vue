...
const { data: page, pending, error } = await useAsyncData(
  () => `environment-page-${route.fullPath}`,
  () => api.request<any>("/api/public/pages/environment/"),
  {
...
const { data: newsData } = await useAsyncData(
  () => `environment-latest-news-${route.fullPath}`,
  () =>
    api.request<NewsListResponse>(
      "/api/public/news/?category=sustainability&tag=environment&page_size=3"
    ),
  {
    watch: [() => route.fullPath],
    default: () => ({
...
<template>
  ...
  >
    <div
      class="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_85%_18%,rgba(34,197,94,0.10),transparent_28%)]"
    />
      ...
      <section v-else class="px-0 lg:px-10">
        ...
        <div v-if="cards.length" class="mt-16 grid gap-6 md:grid-cols-2">
          <div
            v-for="item in cards"
            :key="item.title"
            class="group rounded-[1.6rem] border border-zinc-200 bg-zinc-50 p-7 transition-all duration-500 hover:-translate-y-1 hover:border-emerald-300/40 hover:shadow-[0_0_40px_rgba(34,197,94,0.10)]"
          >
            <p
              class="text-xs font-semibold uppercase tracking-[0.25em] text-emerald-500"
            >
              {{ item.eyebrow }}
            </p>
        ...
        <div v-if="bullets.length" class="mt-20">
          ...
          <div class="mt-8 space-y-5">
            ...
            >
              <div
                class="mt-2 h-3 w-3 rounded-full bg-gradient-to-r from-emerald-400 to-cyan-400"
              />
        ...
        <div v-if="latestNews.length" class="mt-24">
          <div class="mb-10 flex items-end justify-between gap-6">
            ...
            <NuxtLink
              to="/news/topics/environment-news"
