...
<template>
    <main class="min-h-screen overflow-hidden bg-black px-6 pt-30 pb-20 text-white">
        <section class="mx-auto max-w-7xl">
            ...
            <div class="relative columns-1 gap-4 sm:columns-2 lg:columns-3">
                <figure v-for="(image, index) in images" :key="image.id"
                    ...
                    <img :src="image.thumbnail_url || image.file_url || ''"
                        :alt="image.alt_text || image.title || 'Gallery image'"
                        class="w-full object-cover transition duration-500 group-hover:scale-105" 
                        :class="index % 5 === 0
                        ? 'aspect-[4/5]': index % 3 === 0 ? 
                        'aspect-[16/10]': 'aspect-[4/3]'" 
                    />

                    <figcaption v-if="image.title || image.caption"
                        class="bg-gradient-to-t from-black/80 to-transparent p-4">
                        <h3 class="text-sm font-semibold">
                            {{ image.title }}
                        </h3>

                        <p v-if="image.caption" class="mt-1 line-clamp-2 text-xs text-white/60">
                            {{ image.caption }}
                        </p>
                    </figcaption>
                </figure>
        ...
        <!-- preview modal -->
        <div v-if="previewOpen && activeImage" class="fixed inset-0 z-50 bg-black/95 text-white">
            <button
            ...
            <div class="absolute left-0 right-0 top-4 z-10 flex gap-3 overflow-x-auto px-6">
                <button v-for="(image, index) in images" :key="image.id"
                    ...
                    <img :src="image.thumbnail_url || image.file_url || ''" class="h-full w-full object-cover" />
                </button>
            </div>
            ...
            <div v-if="activeImage.title || activeImage.caption"
                ...
                <p v-if="activeImage.caption" class="mt-1 text-xs text-white/60">
                    {{ activeImage.caption }}
                </p>
            </div>
        </div>
